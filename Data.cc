...
bool Data::compare(Data i, Data j, int bin, int n) const {
  ...
  if (diff == 0)
    return true;
  double error = sqrt(
      pow((i.m_data[bin] - j.m_data[bin]) / (abs(diff))*i.m_error[bin], 2) +
      pow((j.m_data[bin] - i.m_data[bin]) / (abs(diff))*j.m_error[bin], 2));
...
// check if two data sets are within n errors of each other and return the
// number that are not
...
Data Data::operator+(Data &in) {
  ...
  if (this->size() != in.size())
    throw std::invalid_argument(
        "Data objects must have the same size for addition.");
  ...
  for (int i = 0; i < this->size(); ++i) {
    ...
    y.m_data[i] =
        (this->measurement(i) * (w_1) + in.measurement(i) * w_2) / (w_1 + w_2);
...
double Data::chi_2() {
  double sum = 0;
  for (int i = 0; i < this->size(); ++i) {
    sum += pow(this->measurement(i) - f_x(this->binCenter(i)), 2) /
           pow(this->error(i), 2);
  }
  return sum / 56;
}
